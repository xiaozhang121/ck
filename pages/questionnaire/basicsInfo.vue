	...
	<view class="box">
		...
		<view class="content">
			<view class="examination">
				<examination :data="list" :count="count" :showProgress="true" @on-next="goNext"></examination>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goNext (data) {
				/* 存入缓存 */
				setStorage('basicsInfo', data)
				
				const URL = '/pages/test/index?num=1'
				uni.redirectTo({
					url: URL
				})
			}
